...
/* 1. DATAFRAME AND LOADING */
  ...
  // p5 v2: returns Promise<DataFrame>
  // p5 v1: keeps old preload/callback behavior
  p5.prototype.loadDataFrame = function(path, callback) {
    const p = this;
    const isP5v2 = typeof p5 !== 'undefined' &&
      typeof p5.VERSION === 'string' &&
      p5.VERSION.indexOf('2.') === 0;

    if (isP5v2) {
      return p.loadTable(path, ',', 'header').then(function(table) {
    ...
    const table = p.loadTable(path, ',', 'header', function() {
  ...
  // If given a path:
  // - p5 v2 returns Promise<DataFrame>
  // - p5 v1 returns a placeholder DataFrame and fills it when loaded
  p5.prototype.tableToDataFrame = function(pathOrTable, type, options) {
    const p = this;
    const delimiter = type || ',';
    const parseOptions = options || 'header';
    const isP5v2 = typeof p5 !== 'undefined' &&
      typeof p5.VERSION === 'string' &&
      p5.VERSION.indexOf('2.') === 0;
    ...
    if (typeof pathOrTable === 'string') {
      if (isP5v2) {
        return p.loadTable(pathOrTable, delimiter, parseOptions).then(function(table) {
          return p.tableToDataFrame(table);
        });
      }

      const df = new this.chart.DataFrame([]);
      
      p.loadTable(pathOrTable, delimiter, parseOptions, function(table) {
...
p5.prototype.hist = function(data, options = {}) {
  ...
  * Creates a geo chart with geographic points
  ...
  // Global wrappers for p5 global mode (needed in some p5 v2 builds)
  if (typeof window !== 'undefined') {
    const chartFns = ['bar', 'pie', 'series', 'scatter', 'hist', 'table', 'geo', 'toPNG', 'toCSV'];

    chartFns.forEach(function(fnName) {
      if (typeof window[fnName] === 'function') return;

      window[fnName] = function() {
        const inst = window._globalP5Instance || (typeof p5 !== 'undefined' ? p5.instance : null);
        if (!inst || typeof inst[fnName] !== 'function') {
          throw new Error(
            'p5.chart: global "' + fnName + '" is not ready. Call it after createCanvas() inside draw().'
          );
        }
        return inst[fnName].apply(inst, arguments);
      };
    });
  }
